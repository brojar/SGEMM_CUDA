...
void runSgemmFullVectorize(int M, int N, int K, float alpha, float* A, float* B,
    float beta, float* C) {
    const uint BK = 8;
    const uint TM = 8;
    const uint TN = 8;
    if (M >= 128 && N >= 128) {
        ...
        dim3 gridDim(CEIL_DIV(N, BN), CEIL_DIV(M, BM));
        dim3 blockDim((BM * BN) / (TM * TN));
        sgemmFullVectorize<BM, BN, BK, TM, TN>
            << <gridDim, blockDim >> > (M, N, K, alpha, A, B, beta, C);
    }
    else {
        // this is a hacky solution to the underlying problem
...
void runSgemmVectorizeResolveBankConflicts(int M, int N, int K, float alpha, float* A,
    float* B, float beta, float* C) {
    const uint BK = 8;
    const uint TM = 8;
    const uint TN = 8;
    if (M >= 128 && N >= 128) {
        ...
        dim3 gridDim(CEIL_DIV(N, BN), CEIL_DIV(M, BM));
        dim3 blockDim((BM * BN) / (TM * TN));
        sgemmVectorizeResolveBankConflicts<BM, BN, BK, TM, TN>
            << <gridDim, blockDim >> > (M, N, K, alpha, A, B, beta, C);
    }
    else {
        // this is a hacky solution to the underlying problem
...
void run_kernel(int kernel_num, int M, int N, int K, float alpha, float *A,
  ...
  case 12:
    ...
    break;
  case 13:
    runSgemmFullVectorize(M, N, K, alpha, A, B, beta, C);
    break;
  case 14:
    runSgemmVectorizeResolveBankConflicts(M, N, K, alpha, A, B, beta, C);
    break;
  default:
